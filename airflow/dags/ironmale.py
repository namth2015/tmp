...
default_args = {
    ...
    'retry_delay': timedelta(seconds=30),  # Chờ 30 giây trước khi thử lại
   # 'retry_delay': 120 , # Chờ 120 giây (2 phút) trước khi thử lại
}
...
with DAG(
    ...
    schedule_interval='@daily', # Lên lịch cho DAG, ở đây đang để daily
    # schedule_interval='@hourly', # Lên lịch cho DAG, ở đây đang để hourly
    catchup=False,
) as dag:
    ...
    query_task = TrinoOperator(
        ...
        sql="insert into hive.ironmale.soul select * from sqlserver.dbo.soul  where hour = 22",  # Replace with your Trino SQL query
